...
const AdminComments = () => {
  ...
  const onBulkDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Đã xóa",
        description: `Đã xóa ${result.deleted}/${result.total} bình luận.${result.failed > 0 ? ` ${result.failed} thất bại.` : ""}`,
      });
      setSelectedIds(new Set());
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Tổng cộng {totalItems} bình luận
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-4 mb-6">
        ...
        {selectedIds.size > 0 && (
          ...
          >
            ...
            Xóa {selectedIds.size} bình luận
          </Button>
        )}
      </div>
            ...
            {items.map((c) => (
              <TableRow key={c.id} className="border-foreground/10">
                ...
                <TableCell className="text-muted-foreground max-w-xs">
                  <span title={c.content}>{truncate(c.content || "", 80)}</span>
                </TableCell>
                <TableCell className="text-muted-foreground max-w-[200px]">
                  <span title={getArticleTitle(c)}>{truncate(getArticleTitle(c), 40)}</span>
                </TableCell>
        ...
        <AlertDialogContent className="rounded-none border border-foreground/20">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <em>"{truncate(deleteTarget?.content || "", 100)}"</em>
              <br />
      ...
      <AlertDialog open={isBulkDeleteOpen} onOpenChange={setIsBulkDeleteOpen}>
        <AlertDialogContent className="rounded-none border border-foreground/20">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn đang xóa <strong>{selectedIds.size}</strong> bình luận đã chọn.
              <br />
              Hành động này không thể hoàn tác.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          {detailComment && (
            <div className="space-y-4 py-2">
              <div>
                ...
                {detailComment.expand?.user?.email && (
                  <p className="text-sm text-muted-foreground">{detailComment.expand.user.email}</p>
                )}
              </div>
              <div>
