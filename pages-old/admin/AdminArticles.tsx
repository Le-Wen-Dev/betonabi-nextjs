...
const AdminArticles = () => {
  ...
  const [categoryInput, setCategoryInput] = useState<string>("all"); // category id
  ...
  // Auto-publish scheduled articles on mount and periodically
  useEffect(() => {
    if (!isAdmin) return;
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        // Best-effort: ensure featured count <= 2 (admin only).
        // This fixes existing bad data where too many records are featured=true.
