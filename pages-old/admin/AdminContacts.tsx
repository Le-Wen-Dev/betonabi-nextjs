...
import {
  listContactSubmissions, deleteContactSubmission,
  deleteContactSubmissions, type ContactRecord,
} from "@/lib/contacts";
          ...
          <TableBody>
            {items.map((c) => (
              <TableRow key={c.id} className="border-foreground/10">
                <TableCell>
                  <input type="checkbox" checked={selectedIds.has(c.id)} onChange={() => toggleSelect(c.id)} className="rounded border-foreground/30" />
                </TableCell>
                ...
                <TableCell className="text-muted-foreground max-w-xs">
                  <span title={c.tinnhan || ""}>{truncate(c.tinnhan || "", 60)}</span>
                </TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-foreground/5" title="Xem chi tiết" onClick={() => setDetailContact(c)}>
                      <Eye className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-foreground/5" title="Xóa" onClick={() => setDeleteTarget(c)}>
          ...
          {detailContact && (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Công ty/Tổ chức</p>
                  <p className="font-medium">{detailContact.tencongty || "—"}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Phòng ban/Chức vụ</p>
                  <p className="font-medium">{detailContact.phongbanchucvu || "—"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p>{detailContact.created ? new Date(detailContact.created).toLocaleString() : "—"}</p>
                </div>
              </div>
              <div>
