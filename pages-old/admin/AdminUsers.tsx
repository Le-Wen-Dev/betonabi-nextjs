...
type UserRecord = {
  ...
  name?: string;
  role?: Role;
  allowedCategories?: string[]; // relation[] (category ids) OR string[]
  verified?: boolean;
  created: string;
  updated: string;
};
...
const AdminUsers = () => {
  ...
  // Debounce search 1s before querying
  useEffect(() => {
  ...
  const fetchUsers = async () => {
    setIsLoading(true);
    try {
      const res = await pb.collection("users").getList<UserRecord>(page, PER_PAGE, {
        sort: "-created",
        filter,
      });
      setItems(res.items);
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        // ưu tiên name_vi nếu có, fallback name
  ...
  const openEdit = (u: UserRecord) => {
    ...
    setFormName(u.name || "");
    setFormRole((u.role || "user") as Role);
  ...
  const selectedCategoryIds = useMemo(() => {
    // chỉ giữ các id còn tồn tại trong categories để tránh lưu rác
  ...
  const isAllSelected = useMemo(() => {
    return allCategoryIds.length > 0 && selectedCategoryIds.length === allCategoryIds.length;
  ...
  const selectAllCategories = () => {
    setFormAllowedCategories(allCategoryIds);
  };
  ...
  const submit = async () => {
    try {
      if (dialogMode === "create") {
        ...
        if (formPassword !== formPasswordConfirm) {
          toast({
            ...
            description: "Vui lòng kiểm tra lại xác nhận mật khẩu.",
            variant: "destructive",
          });
          return;
        }
        ...
        const payload: Record<string, unknown> = {
          ...
          name: formName.trim(),
          role: formRole,
          // PocketBase: email chỉ hiện cho user khác khi emailVisibility=true
        ...
        // emailVisibility thường không set được ngay lúc create (tuỳ PB version/permission).
        // Cố gắng update lại sau create để admin list thấy email.
        try {
          const createdId = (created as { id?: string }).id;
          if (createdId) {
            await pb.collection("users").update(createdId, { emailVisibility: true });
          }
        } catch (e) {
          toast({
            title: "Chưa bật được emailVisibility",
            description:
              "Để admin thấy email của user/author, hãy bật emailVisibility=true cho các record (hoặc set users.manageRule = @request.auth.role=\"admin\").",
          });
        }
      ...
      if (formRole === "author") {
        ...
        // nếu bỏ role author -> clear quyền danh mục
      ...
      if (formPassword || formPasswordConfirm) {
        if (!formPassword || !formPasswordConfirm) {
          toast({
            ...
            description: "Vui lòng nhập đủ mật khẩu và xác nhận mật khẩu.",
            variant: "destructive",
          });
          return;
        }
      ...
      <div className="flex gap-4 mb-6">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            ...
            className="pl-9 pr-9 rounded-none border-foreground/20"
            value={searchInput}
            onChange={(e) => {
        ...
        <div className="w-44">
          <Select
            ...
            onValueChange={(v) => {
              setRoleFilterInput(v as Role | "all");
            }}
          >
          ...
          <TableBody>
            {items.map((u) => (
              <TableRow key={u.id} className="border-foreground/10">
                ...
                <TableCell>
                  <span className={roleBadge(u.role)}>{(u.role || "user").toUpperCase()}</span>
                </TableCell>
                <TableCell className="text-muted-foreground">{new Date(u.created).toLocaleString()}</TableCell>
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-foreground/5" onClick={() => openEdit(u)}>
                      <Pencil className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-foreground/5" onClick={() => onDelete(u)}>
        ...
        <DialogContent className="sm:max-w-[520px] rounded-none border border-foreground/20">
          <DialogHeader>
            <DialogTitle className="font-serif text-2xl">
              {dialogMode === "create" ? "Tạo tài khoản" : "Cập nhật user"}
            </DialogTitle>
          </DialogHeader>
          ...
          >
            {/* Dummy fields to discourage browser/password-manager autofill */}
            <input
              tabIndex={-1}
              aria-hidden="true"
              className="hidden"
                ...
                <Popover open={categoryPickerOpen} onOpenChange={setCategoryPickerOpen}>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className="w-full justify-between rounded-none border-foreground/20"
                          ...
                          {categories.map((c) => {
                            ...
                            return (
                              <CommandItem
                                key={c.id}
                                value={`${label} ${c.slug}`}
                                onSelect={() => toggleAllowedCategory(c.id)}
                              >
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                className="rounded-none border-foreground/20"
                placeholder={dialogMode === "edit" ? "Để trống nếu không đổi" : ""}
              />
            </div>
          ...
          <DialogFooter>
            ...
            <Button className="bg-foreground text-background hover:bg-foreground/90 rounded-none" onClick={submit} type="button">
              Lưu
            </Button>
          </DialogFooter>
        </DialogContent>
