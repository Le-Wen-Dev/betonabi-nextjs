  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Kiểm tra collection `media` và API rules.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleUpload = useCallback(
    async (files: FileList | null) => {
      ...
      try {
        ...
        if (err) {
          toast({
            title: "Một số tệp lỗi",
            description: `${err} tệp không tải được.`,
            variant: "destructive",
          });
        }
      } finally {
    ...
    async (record: MediaRecord) => {
      ...
      try {
        ...
        toast({
          ...
          description: "Kiểm tra quyền API.",
          variant: "destructive",
        });
      }
    },
    ...
    <div className="p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <Button
            className="rounded-none font-medium bg-foreground text-background hover:bg-foreground/90"
            disabled={uploading}
      ...
      <div className="bg-background border border-foreground/20 p-6">
        ...
        {loading ? (
          ...
          <div className="text-muted-foreground py-12 text-center">
            Chưa có file nào. Hãy tải lên từ vùng kéo thả phía trên.
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {items.map((record) => {
                ...
                return (
                  <div
                    key={record.id}
                    className="group border border-foreground/10 overflow-hidden bg-muted/30"
                  >
