...
interface Category {
    ...
    // fallback fields (nếu collection cũ vẫn dùng name/description)
    name?: string;
    description?: string;
    created?: string;
    updated?: string;
...
const AdminCategories = () => {
    ...
    const getUniqueSlug = async (baseSlug: string, excludeId?: string) => {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            try {
                // NOTE: Không dùng getFirstListItem() vì client sẽ gửi skipTotal=1,
                // một số PocketBase server version cũ sẽ trả 400 (Bad Request).
    ...
    const handleSubmit = async () => {
        if (!formData.name_vi || !formData.slug) {
            toast({
                title: "Lỗi",
                description: "Vui lòng điền đầy đủ Tên (VI) và đường dẫn (slug)",
                variant: "destructive",
            });
            return;
        }
        ...
        try {
            ...
            if (editingCategory) {
                await pb.collection(CATEGORIES_COLLECTION).update(editingCategory.id, {
                    ...normalized,
                    slug: uniqueSlug,
                });
                toast({ title: "Thành công", description: "Đã cập nhật danh mục" });
            } else {
                await pb.collection(CATEGORIES_COLLECTION).create({
                    ...normalized,
                    slug: uniqueSlug,
                });
            ...
            <div className="flex gap-4 mb-6">
                <div className="relative flex-1 max-w-md">
                    ...
                    <Input
                        ...
                        className="pl-9 rounded-none border-foreground/20"
                        value={search}
                        onChange={(e) => {
                    ...
                    <TableBody>
                        {categories.map((category, index) => (
                            <TableRow key={category.id} className="border-foreground/10">
                                ...
                                <TableCell className="text-muted-foreground">
                                    {category.updated ? new Date(category.updated).toLocaleString() : "-"}
                                </TableCell>
                    ...
                    <div className="grid gap-6 py-4">
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input
                                ...
                                className="rounded-none border-foreground/20 font-mono text-sm"
                            />
                        </div>
                    </div>
                    <DialogFooter>
