...
const LoginPage = () => {
  ...
  const fromPathname = searchParams.get("from") || undefined;
  const requiredRole = searchParams.get("requiredRole") as LocationState["requiredRole"] | null;
  ...
  }, [ready, isAuthenticated, isAdmin, isAuthor, router]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (u.role === "admin") {
        router.replace(fromPathname || "/admin/dashboard");
        return;
      }

      if (u.role === "author") {
        router.replace(fromPathname || "/admin/articles");
        return;
      }
      ...
      router.replace(fromPathname || "/user/dashboard");
    } catch (err: any) {
      const msg =
