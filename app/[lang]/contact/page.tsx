...
export default function ContactPage() {
  ...
  const [form, setForm] = useState({
    name: '', company: '', department: '', email: '', phone: '', message: '',
  });
  ...
  const inputClass = 'w-full px-3 py-2 border border-gray-300 rounded-sm text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#7c3aed] focus:border-transparent';
  const inputErrorClass = 'w-full px-3 py-2 border border-red-400 rounded-sm text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent';
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          <div>
            ...
            <input type="text" value={form.name} onChange={(e) => handleChange('name', e.target.value)}
              placeholder={no('Nhập họ và tên của bạn', '氏名をご入力ください。')} className={cls('name')} />
            {errorText('name')}
          </div>
          ...
          <div>
            ...
            <input type="text" value={form.company} onChange={(e) => handleChange('company', e.target.value)}
              placeholder={no('Nhập công ty hoặc tổ chức', '会社名または団体名をご入力ください。')} className={cls('company')} />
            {errorText('company')}
          ...
          <div>
            ...
            <input type="text" value={form.department} onChange={(e) => handleChange('department', e.target.value)}
              placeholder={no('Nhập tên phòng ban (không bắt buộc)', '部署名をご入力ください（任意）')} className={cls('department')} />
          ...
          <div>
            ...
            <input type="text" value={form.email} onChange={(e) => handleChange('email', e.target.value)}
              placeholder={no('Nhập địa chỉ email của bạn', 'メールアドレスをご入力ください。')} className={cls('email')} />
            {errorText('email')}
          ...
          <div>
            ...
            <input type="tel" value={form.phone} onChange={(e) => handleChange('phone', e.target.value)}
              placeholder={no('Nhập số điện thoại (không bắt buộc)', '電話番号をご入力ください（任意）')} className={cls('phone')} />
            {errorText('phone')}
          </div>
