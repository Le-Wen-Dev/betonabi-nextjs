...
export default function UserDashboard() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        setSavedItems([]); setSavedPage(1); setSavedTotalPages(1);
        setMyComments([]); setCommentPage(1); setCommentTotalPages(1);
      } finally {
  ...
  const loadMoreSaved = async () => {
    if (!user?.id || savedLoadingMore || savedPage >= savedTotalPages) return;
  ...
  const loadMoreComments = async () => {
    if (!user?.id || commentLoadingMore || commentPage >= commentTotalPages) return;
  ...
  const savedList = useMemo(() => {
    return savedItems.map((s) => {
      ...
      const dateStr = a?.publishedAt || (a as any)?.created || s.created || '';
  ...
  const myCommentList = useMemo(() => {
    return myComments.map((c) => {
      ...
      const title = language === 'JP' ? (a?.title_jp || a?.title_vi || (a as any)?.title || '') : (a?.title_vi || a?.title_jp || (a as any)?.title || '');
      const slug = a?.slug || a?.id;
                ...
                {savedList.map((a) => (
                  <div key={a.id} className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <Link href={localePath(locale, `/longform/${a.slug}`)} className="font-medium hover:underline line-clamp-2">
                        {a.title}
                      </Link>
                      <div className="text-xs text-muted-foreground mt-1">{a.date}</div>
                    </div>
                    <Link href={localePath(locale, `/longform/${a.slug}`)} className="text-sm text-muted-foreground hover:text-foreground">
                      {t('viewMore')}
                    </Link>
                  </div>
                ))}
              ...
              <div className="space-y-4">
                {myCommentList.map((c) => (
                  <div key={c.id} className="border border-border p-4 rounded-sm">
                    <Link href={localePath(locale, `/longform/${c.slug}`)} className="font-medium hover:underline line-clamp-2">
                      {c.articleTitle}
                    </Link>
                    <div className="text-xs text-muted-foreground mt-1">{c.time}</div>
                    <div className="mt-2 text-sm text-foreground/90 whitespace-pre-wrap line-clamp-4">{c.content}</div>
                  </div>
                ))}
              </div>
            ) : (
